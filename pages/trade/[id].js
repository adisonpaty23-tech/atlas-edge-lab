...
export default function TradeDetail() {
  ...
  const avgEdgeScore = useMemo(() => {
    ...
    return Number(
      (
        allTrades.reduce((sum, t) => sum + getEdgeScore(t), 0) / allTrades.length
      ).toFixed(1)
    )
  }, [allTrades])
  ...
  const scoreComparison = useMemo(() => {
    if (!allTrades.length) return "Need more data"
    if (edgeScore > avgEdgeScore) return "Above your average setup quality"
    if (edgeScore < avgEdgeScore) return "Below your average setup quality"
    return "In line with your average setup quality"
  }, [edgeScore, avgEdgeScore, allTrades])
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1.35fr 0.95fr", gap: "20px", marginBottom: "20px" }}>
          ...
          <div style={whiteCardLarge}>
            ...
            <div style={{ display: "grid", gap: "16px" }}>
              ...
              <div style={loopCard}>Average Edge Score: {avgEdgeScore}/10</div>
              <div style={loopCard}>{scoreComparison}</div>
              <div style={loopCard}>
                Most similar winning trade: {mostSimilarWinning ? `${mostSimilarWinning.pair} (${mostSimilarWinning.similarity}%)` : "Need more data"}
              </div>
            </div>
          </div>
        </div>
